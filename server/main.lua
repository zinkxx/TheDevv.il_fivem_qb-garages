...
QBCore.Functions.CreateCallback("qb-garage:server:GetGarageVehicles", function(source, cb, garage, garageType, category)
    ...
    elseif garageType == "depot" then    --Depot give player cars that are not in garage only
        MySQL.Async.fetchAll('SELECT * FROM player_vehicles WHERE citizenid = ? AND (state = ? OR state = ? OR garage = ?)', {pData.PlayerData.citizenid, 0, 2, garage}, function(result)            local tosend = {}
            if result[1] then
        ...
        MySQL.Async.fetchAll('SELECT * FROM player_vehicles WHERE state = ?'..shared, {1}, function(result)
            if result[1] then
                cb(result)
            else
                cb(nil)
...
QBCore.Commands.Add("restorelostcars", "Konfigürasyonda artık mevcut olmayan veya geçersiz olan bir garaja park edilmiş arabaları geri yükler (isim değişikliği veya kaldırıldı).", {{name = "destination_garage", help = "(Opsiyonel) Arabaların gönderildiği garaj."}}, false,
